...
{   
    ...
    Eigen::Vector3f c(_v[2].x()-_v[0].x(), _v[2].y()-_v[0].y(), 0);

    Eigen::Vector3f aa(x-_v[0].x(), y-_v[0].y(), 0);
    Eigen::Vector3f bb(x-_v[1].x(), y-_v[1].y(), 0);
    Eigen::Vector3f cc(x-_v[2].x(), y-_v[2].y(), 0);

    // judge whether the 3 cross products are the same direction
    if (a.cross(aa).dot(b.cross(bb)) < 0.0) return false;
    if (c.cross(cc).dot(b.cross(bb)) < 0.0) return false;
    if (a.cross(aa).dot(c.cross(cc)) < 0.0) return false;
    return true;
}
